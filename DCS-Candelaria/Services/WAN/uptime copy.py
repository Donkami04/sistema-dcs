...
def format_historic_data(data):
    logging.info(f"Data entrante: {data}")
    logging.info(f"LENGTH Data entrante: {len(data)}")
    if data == '100':
    ...
    logging.info(f"Data formateada en format_historic_data: {data}")
    return data
...
def get_wan_data(sensor_ping_id, sdate, edate):
    ...
    logging.info(f"uptime_percent: {uptime_percent}")
    logging.info(f"downtime_percent: {downtime_percent}")
